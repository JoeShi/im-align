...
- Requires CI secrets: Feishu app credentials, test-group chat, the rotating `IM_ALIGN_E2E_FEISHU_USER_REFRESH_TOKEN`, and the restricted `IM_ALIGN_E2E_GITHUB_SECRETS_PAT`. A pre-test helper exchanges and rotates the refresh token, exposing `IM_ALIGN_E2E_USER_ACCESS_TOKEN` only through the current job's `GITHUB_ENV`. The actor and verifier are both built from the user Participant identity; Bridge app credentials are never used to read Thread history or reactions. Threads are never cleaned up; they are the audit trail.
...
- The runner (`tests/e2e/smoke_runner.py`, entry `python -m tests.e2e.smoke_runner <scenario_dir> [--backend kiro-cli] [--participant-mode as-user|bot|all]`) drives the whole run: seed materialization, Bridge start with the scenario's `spec_root` in the isolated user config, the User Simulator, deterministic assertions, transcript export, evaluator, and rubric archiving. The isolated user config also carries `defaults.timeouts.turn_timeout_seconds` set to the scenario's `timeout_seconds`: measured 2026-09-16, a real opencode Turn exceeded the 300 s product default and was cancelled mid-exploration, leaving a question-less partial card that stalled the Session until the scenario timeout. One Turn may therefore use the whole scenario budget; the runner's wall-clock wait bounds the run overall. `as-user` is the supported acceptance mode. `bot` and `all` remain available only for the ADR-0009 platform re-probe; bot runs are expected to fail acknowledgement and Turn-driving assertions until bot-originated event delivery is re-validated. LLM credentials come from `SIMULATOR_LLM_*` and `EVALUATOR_LLM_*`. Anything missing skips only the affected tuple with an explicit report.
- `tests/e2e/participants.py` is the Participant Adapter seam. Its `as-user` and `bot` adapters hide authentication, bot open_id resolution, Bridge mention formatting, and allowlist contribution behind the same Thread interface. Explicit open_id environment values are optional overrides. Every simulator reply returns a receipt. Deterministic assertions require at least one visible reply, an emoji ack on every reply, and a later Bridge-authored Thread message proving that the reply drove a Turn.
- The bot adapter remains an ADR-0006 compatibility and platform-diagnostic seam. Its dedicated app uses `references/lark-e2e-simulator-scopes.json`; in particular, `im:message.group_msg` is required for its polling transport. The allowlist injection remains defense in depth, but the current platform does not deliver its replies to the Bridge event stream. Participant thread failures are propagated to the runner and stop the Session so missing permissions fail promptly instead of waiting for the scenario timeout.
...
- LLM access for simulator and evaluator is configured via environment variables (`SIMULATOR_LLM_BASE_URL`, `SIMULATOR_LLM_API_KEY`, `SIMULATOR_LLM_MODEL`; same for `EVALUATOR_LLM_*`), injected from CI secrets. No provider is hardcoded.
...
- Same Scenario schema and same runner as Backend Smoke; the supported matrix iterates Agent Backends × Scenarios in `as-user` mode (`python -m tests.e2e.smoke_runner <dir>... --backends all --participant-mode as-user`). `bot` or `all` may be selected only for the separate platform-delivery diagnostic and are not expected green under ADR-0009.
...
- `.github/workflows/backend-smoke.yml`: nightly (or manual from `refs/heads/main`) Backend Smoke across all Agent Backends in supported `as-user` mode. The credentialed job refuses other refs. Bot mode is not normalized into the green nightly matrix; re-probe it explicitly when platform delivery may have changed.
...
- Harness-only credentials (`IM_ALIGN_E2E_*`, `SIMULATOR_LLM_*`, and `EVALUATOR_LLM_*`) are removed from the Bridge worker environment before it launches an Agent Backend. `IM_ALIGN_E2E_USER_ACCESS_TOKEN` is held only by the harness-side Participant adapter and is never written to repository config, user Bridge config, Session state, logs, or archives.
