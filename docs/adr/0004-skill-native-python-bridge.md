...
- **Bridge orchestrates send/receive through lark-cli**: smaller implementation but too coupled to Feishu/Lark. lark-cli remains only an optional identity probe.
- **Browser OAuth or claim-confirm strong identity gate**: stronger guarantee but high cost. v1 uses a lightweight identity chain while Approval/stop callbacks still strictly compare open_id values.
...
- **Identity chain**: repository config -> optional lark-cli from the same Feishu/Lark app -> git email -> interactive email. Email is resolved through contact API to an open_id under the Bridge app. Feishu/Lark open_id values are app-scoped, so lark-cli identities from other apps must not be reused directly. Approval and stop hard-check open_id.
